...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-lg-12">
                {{-- <div class="p-5"> --}}
                    <form id="frmDatos" method="POST" action="{{route('admin.roles.update', $role)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group row">
                            <div class="col-sm-12 mb-3 mb-sm-0">
                                <label for="nombre" class="form-label">Nombre</label>
                                <input type="text" class="form-control" id="name" name="name" placeholder="Ingrese el nombre del rol" required value="{{$role->name, old('nombre')}}">
                            </div>
                        </div>
                        ...
                        @foreach ($permissions as $permission)
                            ...
                            <div>
                                <label>
                                    <input type="checkbox" class="mr-1" value="{{ $permission->id }}" {{$role->hasPermissionTo($permission->id) ? 'checked' : '' }} name="permissions[]"> 
